...
public class TitleFlowIndicator extends TextView {
	...
	private static final int SELECTED_COLOR = Color.rgb(255, 117, 117);
	private static final boolean SELECTED_BOLD = false;
	private static final int TEXT_COLOR = Color.rgb(102, 102, 102);
	private static final int TEXT_SIZE = 25;
	private static final float FOOTER_LINE_HEIGHT = 4.0f;
	private static final int FOOTER_COLOR =Color.rgb(255, 117, 117);
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int iLoop = 0; iLoop < count; iLoop++) {
			// Get the title
			String title = getTitle(iLoop,mode);
			Rect bound = bounds.get(iLoop);
	...
	private Rect calcBounds(int index, Paint paint) {
		// Get the title
		String title = getTitle(index,mode);
	 ...
	 * @return
	 */
	private String getTitle(int pos,int mode) {
		if(mode==0)
		return Data.Gconn_title[pos];
		else 
		return Data.VW_title[pos];
	}
